...
(async () => {
  ...
  // Get all projects
  let projectsRef = db.collection('popular');
  ...
  projects.forEach(project => {
    let reviewsRef = db.collection('popular').doc(project.uid).collection('comments');
    // Make reviews
    let numReviews = generateValueBetweenMinAndMax(0, 100), usersCopy = JSON.parse(JSON.stringify(users.users)), userStart = null, userId = 0;
